...
@Component({
  ...
  styleUrls: ['./point-list.page.css']
})
export class PointListPage extends BSDatatableController {
    ...
    onClickExpirationProcess() {
        this.alert.confirm('만료처리를 진행하시겠습니까?').subscribe((result) => {
            this.pointStore.expiration().subscribe(resp => {
                console.log('pointChargeStore.expiration resp =>', resp);
