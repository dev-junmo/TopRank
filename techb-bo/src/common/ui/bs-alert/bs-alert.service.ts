...
export class BSAlertService {
    ...
    public show(message, title = '', okBtnText = '확인') {
        ...
        this.bsModalRef = this.modalService.show( BSAlertComponent,
            { ignoreBackdropClick: true, initialState, class: 'alert'});
    ...
    public confirm(message, title = '확인', okBtnText = '확인', cancelBtnText = '취소', needCancelFeedback = false) {
        ...
        this.bsModalRef = this.modalService.show( BSAlertComponent, {initialState , class: 'alert'});		// entryComponent

        return subject.asObservable();
    }

    confirmWithInputMessage(message, title = '확인', okBtnText = '확인', cancelBtnText = '취소', needCancelFeedback = false) {
        const subject = new Subject<any>();

        const initialState = {
            useAppendMessage: true,
            title: title,
            message: message,
            btnOkText: okBtnText,
            btnCancelText: cancelBtnText,
